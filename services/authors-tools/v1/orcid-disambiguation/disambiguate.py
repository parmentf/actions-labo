...
class disambiguate:
    ...
    def getDfFromName(self, name):
        url = "https://pub.orcid.org/v3.0/csv-search/?q=" + urllib.parse.quote(
            "(given-and-family-names:" + name + ")"
        )
    ...
    def extractDfInfos(self, df):
        ...
        for i in range(self.nameDepth):
            ...
            try:
                dic["affiliations"] = df["current-institution-affiliation-name"][
                    i
                ].split(",")
            except Exception:
                dic["affiliations"] = []
    ...
    def extractInfoFromWorks(self, works):
        ...
        for i, work in enumerate(works):
            ...
            for ws in workSummary:
                ...
                try:
                    dic["externalIds"].append(
                        [
                            external_id["external-id-value"]
                            for external_id in ws.get("external-ids")["external-id"]
                        ]
                    )
                except Exception:
    ...
    def splitFirstName(self, name):
        ...
        if len(fnSplit) > 1:  # composed firstname with - in it
            choice.append(
                nameSplit[1] + ", " + fnSplit[0][0] + ".-" + fnSplit[1][0] + "."
            )
            choice.append(
                nameSplit[1] + ", " + fnSplit[0][0] + "." + fnSplit[1][0] + "."
            )
            ...
            if len(fnSplit) > 1:
                choice.append(
                    nameSplit[1] + ", " + fnSplit[0] + ".-" + fnSplit[1][0] + "."
                )  # lastname, Jaa.-P.
                choice.append(
                    nameSplit[1] + ", " + fnSplit[0] + " " + fnSplit[1][0] + "."
                )  # lastname, Jaa P.
                ...
                choice.append(
                    fnSplit[0] + " " + fnSplit[1][0] + ". " + nameSplit[1]
                )  # Jaa P. lastname
                choice.append(
                    fnSplit[0][0] + "." + fnSplit[1][0] + ". " + nameSplit[1]
                )  # J.P. lastname
                choice.append(
                    fnSplit[0][0] + fnSplit[1][0] + ". " + nameSplit[1]
                )  # JP. lastname
    ...
    def disambiguation(self):
        df = self.getDfFromName(
            self.infoDic["firstName"] + "+AND+" + self.infoDic["lastName"]
        )
        ...
        for personInfos in personsInfos:
                ...
                for title in self.infoDic["titles"]:
                    for tit in [title.lower() for title in worksInfo["title"]]:
                        ratio = SequenceMatcher(
                            None, title.lower(), tit
                        ).ratio()  # check similarity between title
                        if ratio > 0.7:
            ...
            if "affiliations" in self.infoDic:  # check affiliations
                for affiliation in self.infoDic["affiliations"]:
                    if affiliation.lower() in [
                        aff.lower() for aff in personInfos["affiliations"]
                    ]:
            ...
            if self.infoDic["lastName"].lower() == personInfos["lastName"].lower():
                if (
                    self.infoDic["firstName"].lower()
                    == personInfos["firstName"].lower()
                ):
                    points += 10
                    matchArg.append("Match First+LastName ")
                elif (
                    self.infoDic["firstName"].lower()[0]
                    == personInfos["firstName"].lower()[0]
                ):
                    points += 7
