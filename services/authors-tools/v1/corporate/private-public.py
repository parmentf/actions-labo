...
def request(name, dept):
    url = "https://recherche-entreprises.api.gouv.fr/search?q=" + urllib.parse.quote(
        name
    )
    if dept:
...
def is_private_public(information):
    if (
        len(information) == 0
        or "results" not in information
        or not information["results"]
    ):
...
def main():
    my_dict = request_abbreviation(
        "http://mapping-tables.daf.intra.inist.fr/affiliations-tools-corporate.json"
    )
